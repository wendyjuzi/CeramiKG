        ...
        if key not in self._models:
            ...
            if backend == "transformers":
                ...
                if backend == "vllm-engine":
                    ...
                    # Enforce eager execution to avoid torch.compile (torch._dynamo)
                    # tracing generators inside the model which causes:
                    # "torch._dynamo.exc.Unsupported: Attempt to trace generator".
                    # Remove this when upstream vllm/torch compilation issues are fixed.
                    ...
                    if "model" not in kwargs:
                        kwargs["model"] = model_path
                    # ------------------------------------------------------------------
                    # TEMPORARY WORKAROUND (async): enforce eager execution for same reason
                    # as synchronous vllm backend above. Remove after upstream fixes.
                    if "enforce_eager" not in kwargs:
                        kwargs["enforce_eager"] = True
                    # ------------------------------------------------------------------
                    if enable_custom_logits_processors() and ("logits_processors" not in kwargs):
